...
class StatsCommand(discord.Cog, name="stats"):
    ...
    async def cmd_leaderboard(
        ...
        if leaderboard.lower() == "top helpers (messages)" or leaderboard.lower() ==  "top helpers (threads)":
            ...
            new_threads = [thread for thread in new_threads if thread["time"] > 0]
            ...
            return await inter.respond(f"{leaderboard.lower()} is an invalid leaderboard type.")
    ...
    user = discord.SlashCommandGroup("user",parent=stats)
