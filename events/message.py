...
class OnMessage(commands.Cog):
    ...
    async def on_message(self, message: disnake.Message):
        if type(message.channel) is disnake.Thread and message.channel.parent.get_tag_by_name("RESOLVED") in message.channel.applied_tags and not (message.author.id == self.bot.user.id):
            await message.channel.remove_tags(message.channel.parent.get_tag_by_name("RESOLVED"))
