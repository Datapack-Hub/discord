...
async def get_member_join_card(user,self):
    ...
    draw.ellipse((offset, offset, pfp_image.size[0] - offset, pfp_image.size[1] - offset), fill=255)
    ...
    if width > 634:
        for i in range (0,int(width)):
            width = draw.textlength(name, font=ImageFont.truetype(font_path,size=80-i))
            if width <= 634:
                bbox = draw.textbbox((1, 1),name.replace("y","").replace("g","").replace("j",""), font=ImageFont.truetype(font_path,size=80-i))
                height = bbox[3] - bbox[1]

                draw.text(((265 + (581 / 2) - (width / 2)+10), 87+61-height), name, font=ImageFont.truetype(font_path,size=80-i), fill=(255, 99, 26))
                break
    else: 
        draw.text(((265 + (581 / 2) - (width / 2)+10), 87), name, font=font, fill=(255, 99, 26))

    width = draw.textlength("Member #"+str(member_count), font=font_2)
    bbox = draw.textbbox((1, 1),"Member #"+str(member_count), font=font_2)
    height = bbox[3] - bbox[1]
    draw.text(((265 + (581 / 2) - (width / 2)), 187), "Member #"+str(member_count), font=font_2, fill=(255, 166, 74)) 
...
class OnMemberJoin(commands.Cog):
    ...
    async def on_member_join(self, member: disnake.Member):
        ...
        os.chdir(script_dir)
        directory = os.getcwd()
        await channel.send(file = disnake.File(os.path.join(directory, 'files', 'output.png')))