...
class SiteCommand(commands.Cog, name="site"):
    ...
    async def search(self, inter: disnake.ApplicationCommandInteraction, query: str):
        out = requests.get(
            f"https://api.datapackhub.net/projects/search?query={query}"
        ).json()
        results = ""
        for i in out["result"]:
    ...
    @site.sub_command(
        "project",
        description="View details about a DPH Project",
        options=[
            disnake.Option(
                "slug", "The slug (URL) of the datapack hub project.", required=True
            )
        ],
    )
    async def project(self, inter: disnake.ApplicationCommandInteraction, slug: str):
        ...
        emb = (
            ...
            .set_author(
                ...
                url=f"https://datapackhub.net/user/{out['author']['username']}",
                icon_url=out["author"]["profile_icon"],
            )
            .add_field("Updated", f"<t:{out['updated']!s}:D>")
