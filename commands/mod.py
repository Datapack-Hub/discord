...
class ModCommand(commands.Cog):
    ...
    async def info(self, inter: disnake.ApplicationCommandInteraction):
        ...
        body = "This list is generated by counting the amount of messages sent by users in help threads which they did not create. It only uses data from currently open and recent help threads.\n\n"
        
        index = 1
        for helper in helper_data[:30]:
            percentage = round((helper["count"] / total) * 100, 1)
            body += f"{index!s}. **{helper['username']}**: `{percentage!s}%` ({helper['count']})\n"
            index += 1
        
        if len(helper_data) > 30:
            body += f"\n{len(helper_data - 30)!s} users were hidden from this list for having low message counts."
