...
class DailyStats(commands.Cog):
    ...
    async def daily(inter: disnake.ApplicationCommandInteraction):
        ...
        joined_today = len([member for member in inter.guild.members if member.joined_at.date() == today])
        ...
        for channel in v.help_channels:
            ...
            threads += len([thread for thread in channel.threads if thread.created_at.date() == today])
            
        current_date_as_text = today.strftime("%d/%m/%y")
        past_data.append({"Date":current_date_as_text,"Amount":len(inter.guild.members)})
    ...
    async def each_day(self):
        ...
        past_data = json.loads(open("member_data.json","r").read())
        today = datetime.now().date()
        joined_today = len([member for member in members if member.joined_at.date() == today])
